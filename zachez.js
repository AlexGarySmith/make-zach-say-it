...
const AUTOPLAY_MEDIA_SELECTOR = '#pageAutoplayMedia';
const ZIMAGE_LIST = [];
...
function initializeAutoplayMedia() {
    const media = document.querySelector(AUTOPLAY_MEDIA_SELECTOR);
    if (!media) return;

    const mediaSource = media.currentSrc || media.getAttribute('src');
    ...
    if (playAttempt && typeof playAttempt.catch === 'function') {
        playAttempt.catch(() => {
            // Autoplay with sound is browser-controlled; ignore blocked attempts.
        });
    }
}
